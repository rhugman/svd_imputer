...
class TestImputerValidation:
    ...
    def valid_data(self):
        ...
        return pd.DataFrame({
            ...
            "B": [2, np.nan, 6, 8, 10, np.nan, 14, 16, 18, 20]
        }, index=dates)
...
class TestImputerUncertainty:
    ...
    def test_uncertainty_different_strategies(self, uncertainty_data):
        ...
        with warnings.catch_warnings():
            ...
            uncertainty_random = imputer.estimate_uncertainty(
                n_repeats=3, mask_strategy="random", frac=0.1
            )
            assert isinstance(uncertainty_random, dict)
            
            # Test block strategy
            uncertainty_block = imputer.estimate_uncertainty(
                n_repeats=3, mask_strategy="block", n_blocks=1, block_len=2
            )
...
class TestImputerErrorHandling:
    ...
    def test_svd_failure_recovery(self):
        ...
        problematic_data = pd.DataFrame({
            "A": [1e-15] * 5 + [1, 2, 3, 4, 5],  # Near-zero values
            "B": [1e15] * 5 + [6, 7, 8, 9, 10]   # Very large values
        }, index=dates)
