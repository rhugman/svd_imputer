...
class TestPublicAPI:
    ...
    def test_api_backwards_compatibility(self):
        ...
        df = pd.DataFrame({
            "A": [1, np.nan, 3, 4, np.nan, 6, 7, 8, 9, 10],
            "B": [2, 4, np.nan, 8, 10, 12, np.nan, 16, 18, 20]
        }, index=dates)
...
class TestIntegrationExamples:
    ...
    def test_uncertainty_estimation_example(self):
        ...
        df = pd.DataFrame({
            "A": np.arange(20) + np.random.randn(20) * 0.1,
            "B": np.arange(20) ** 0.5 + np.random.randn(20) * 0.1
        }, index=dates)
...
class TestPerformanceBenchmarks:
    ...
    def test_memory_usage(self):
        ...
        df = pd.DataFrame({
            f"col_{i}": np.random.randn(200) for i in range(5)
        }, index=dates)
